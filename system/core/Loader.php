...
class CI_Loader {
	...
	public function model($model, $name = '', $db_conn = FALSE)
	{
		...
		foreach ($this->_ci_model_paths as $mod_path)
		{
			...
			$attr = array_keys(get_class_vars($model));
		
			$attrkeys = (get_class_vars($model));

			
			//Model  Change
			$modelattr = $this->_ci_shared->$model;
			if(!($modelattr) || array_diff ($modelattr,$attr) || array_diff ($attr,$modelattr)  ){
			
				//Check Table
					$this->database();
					$this->dbforge();
					if ($CI->db->table_exists(strtolower($model)))
					{
  						$fields = $CI->db->list_fields($model);

						$add = array_diff($attr,$fields);
						...
						foreach($add as $field){
							...
							if(is_numeric($attrkeys[$field])){
								...
								$columns[$field] = array('type' => 'DOUBLE');
							}
							if(is_string($attrkeys[$field])){
								
								$columns[$field] =  array('type' => 'TEXT');
							}
							if($this->validateDate($attrkeys[$field])){
								
								$columns[$field] =  array('type' => 'DATE');
							}
						}
						
						$CI->dbforge->add_column($model, $columns);
				...
				$this->_ci_shared->$model=$attr;
			}else{
				
			}
			
			$methods = get_class_methods($CI->$name);
			
		$arr = Array();
		foreach ($methods as $method ) {
			if(substr($method,0,7)=="onfire_" ){
				...
				$reflection = new ReflectionClass(get_class($CI->$name));
   				 $closure = $reflection->getMethod($method)->getClosure($CI->$name);
				$CI->attach($method, $closure,$params 	);
